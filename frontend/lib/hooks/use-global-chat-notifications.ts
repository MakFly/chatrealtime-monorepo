...
import { useMercureTokenV2 } from './chat-v2/use-mercure-token-v2'
import { useCurrentUser } from './use-current-user'
import { toast } from 'sonner'
import { usePathname, useRouter } from 'next/navigation'
import type { ChatRoomV2Collection, ChatRoomV2 } from '@/types/chat-v2'
 ...
 *
 * Features:
 * - Subscribes to /chat-v2/unread/user/{userId} globally
...
export function useGlobalChatNotifications(
  ...
  const getCurrentRoomId = useCallback((): number | null => {
    ...
    if (!pathname.startsWith('/chat-v2')) {
      return null
    }
  ...
  const handleUnreadUpdate = useCallback(
    (update: UnreadUpdateV2) => {
      ...
      const chatUrl = `/chat-v2?productId=${room.productId}&userId=${sellerUserId}`
