 ...
 * - Subscribes to Mercure for real-time updates on /marketplace-chat/room/{roomId}
...
export function useChatMessagesV2(options: UseChatMessagesV2Options) {
  ...
  const {
    ...
    // When user navigates from /marketplace to /marketplace-chat, new messages may have arrived
  ...
  // ✅ CRITICAL: Only connect if roomId > 0 to prevent /marketplace-chat/room/0 subscription
